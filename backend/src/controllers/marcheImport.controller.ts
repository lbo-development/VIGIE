...
import * as marcheImportService from '../services/marcheImport.service.js'
import { AppError } from '../middlewares/errorHandler.js'

/**
 * Couche HTTP : traduit requête <-> réponse. Le fichier arrive en
 * multipart/form-data (champ "fichier", voir routes/marcheImport.routes.ts —
...
export async function postPreview(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const report = await marcheImportService.preview(req.matricule ?? null, idService, req.file.buffer)
    res.json(report)
  } catch (err) {
    next(err)
  }
...
export async function postConfirm(req: Request, res: Response, next: NextFunction) {
  try {
    if (!req.file) throw new AppError('Aucun fichier reçu.', 400)
    const idService = parseIdService(req)
    const report = await marcheImportService.confirm(req.matricule ?? null, idService, req.file.buffer)
    res.json(report)
  } catch (err) {
    next(err)
  }
...
export async function getLastImport(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const info = await marcheImportService.getLastImportInfo(req.matricule ?? null, idService)
    res.json(info)
  } catch (err) {
    next(err)
  }
