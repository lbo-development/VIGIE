...
export function Reglages() {
  ...
  return (
    <div className="stack">
      ...
      <div className="gp-field" style={{ maxWidth: 400 }}>
      ...
      {selectedKey && (
        <>
          <div className="page-actions">
            <button className="gp-btn gp-btn--primary" onClick={() => setModalOpen(true)}>
              <svg className="ti">
                <use href="#i-plus" />
              </svg>
              Nouvelle valeur
...
function ParametreFormModal({ parametreKey, services, directions, onClose, onSaved }: ParametreFormModalProps) {
  ...
  const [valeur, setValeur] = useState(String(parametreKey.defaut))
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (scope === 'direction' && !idDirection) {
      setError('Choisis une direction.')
      return
    }
    if (scope === 'service' && !idService) {
    ...
    try {
      await api.put(`/parametres/${encodeURIComponent(parametreKey.cle)}`, {
        ...
        idDirection: scope === 'direction' ? Number(idDirection) : null,
        idService: scope === 'service' ? Number(idService) : null,
      })
      onSaved()
    } catch (err) {
      ...
      <div className="gp-modal" role="dialog" aria-modal="true" aria-labelledby="parametreModalTitle">
        <div className="gp-modal__hd">
          <h3 className="gp-modal__title" id="parametreModalTitle">
            Nouvelle valeur — {parametreKey.libelle}
          </h3>
