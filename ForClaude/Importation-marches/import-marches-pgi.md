...
> **Statut** : toutes les décisions d'architecture et de détail sont actées (dernière mise à
> jour 30/08/2026) — voir §9 (historique) pour le détail des choix. Deux migrations de
> schéma restent à exécuter avant implémentation (§3 : `DTELASTIMPORT`,
> `PLANPREVENTIONACTIF`, `ACTIF`/`COMPLETUDE`/`UTILISABLE`) ; deux répercussions sur d'autres
> documents restent à faire (MOT/SECURITY.md pour l'import, MCT/MCD pour `UTILISABLE` et le
> renommage `ETATMARCHE → ACTIF`).
...
| `TYPE_CREATION` | `PGI` | **Jamais réécrit** | Marque l'origine de la fiche, figée après création. **Une création manuelle de marché est prévue** (30/08/2026, écran pas encore construit) — `TYPE_CREATION` prendra alors une seconde valeur, non encore nommée (par analogie avec `FOURNISSEUR.TYPE_CREATION`, `'SERVICE'` est une proposition, à confirmer le moment venu) |
| `TYPEPROC` | Dérivé de `NUMMARCHE` : préfixe `P` → `MAPA`, préfixe `M` → `MARCHE` | **Recalculé, sans effet pratique** (`NUMMARCHE` ne change jamais pour un marché donné, donc le résultat est identique) | Règle calculée, pas une constante ; le fichier modèle ne contient que des numéros préfixés `P`/`M`/`S` (voir ligne 631, `S2109325`) — le cas d'un préfixe hors `P`/`M` n'est pas couvert, à traiter en anomalie lors de l'implémentation |
...
| `TITULAIRE_SERVICE` | `NULL` | **Jamais réécrit** | idem |
...
| `ALERTEMT` | `120` | **Jamais réécrit** | **Seuil en % d'un montant de référence** (ex. `MTMAXI`) déclenchant une alerte budgétaire — **pas un montant brut en €**, malgré ce que le nom du champ suggère. Décision du 29/08/2026 : le MLD (`finances.marche`, §2.2) devra être corrigé pour clarifier ce point (aucun type/unité n'y était précisé jusqu'ici) |
| `ALERTEDATE` | `0.8` | **Jamais réécrit** | **Seuil en % de la durée écoulée** entre `DTEDEBUT` et `DTEFINMAX` déclenchant une alerte — **pas une date calendaire**, même remarque de clarification MLD que ci-dessus |
...
(`ForClaude/CDC/mot-phases-1-2.md` l.71). **Aucune modification du MOT ni du MCT nécessaire
sur ce point** — la formulation initiale (« service porté par admin_app ») était imprécise ;
ADMIN_APP est un rôle transverse sans service propre et n'est pas concerné par cet import.
Aucun croisement avec la cellule `Activité` du fichier (§2).

Vérification d'autorisation côté backend : même patron que les autres écritures scopées par
service (`assertManagesService`, `backend/src/services/authorization.service.ts`) — ADMIN_APP
reste autorisé par cohérence avec ce helper (transverse), mais l'usage principal visé est
ADMIN_SERVICE sur son propre service.

# 5. Étape d'intégration — décidé : reprend OP3.1 telle quelle

**Décision du 29/08/2026 : pas d'étape de confirmation intermédiaire.** L'import reste, comme
le documente déjà OP3.1 (MCT), une **routine automatique embarquant tous les contrôles, sans
étape de confirmation** — la « génération du fichier des opérations à réaliser » évoquée dans
la description initiale **ne bloque pas** l'import : elle correspond en réalité au
**compte-rendu final déjà prévu par OP3.1** (point 5 : « marchés ajoutés, archivés,
fournisseurs ajoutés, anomalies »), produit **après** l'intégration, pas avant. Aucune
modification d'OP3.1 nécessaire — cette description le confirme et le complète (variante
marchés) plutôt qu'elle ne le contredit.
...
3. **Intégration** : ajout (nouveau `NUMMARCHE`, tous les champs valorisés selon §3 —
   `ACTIF = TRUE`, `COMPLETUDE = FALSE`), ou **modification** (`NUMMARCHE` déjà existant) —
   dans ce cas, **précision du 29/08/2026 : seuls les champs ayant une correspondance
   directe avec une colonne du fichier (§3, tableau A-M) sont réécrits ; tous les autres
   champs de la ligne existante restent strictement inchangés**, y compris s'ils ont été
   modifiés manuellement depuis leur dernière valeur d'import (voir détail champ par champ,
   §3). Ou archivage (`NUMMARCHE` absent du fichier mais présent en base pour ce service,
   **et `TYPE_CREATION = PGI`**, précision du 30/08/2026 → `ACTIF = FALSE`, jamais de
   suppression physique — un marché créé manuellement n'est jamais désactivé de cette façon.
   L'archivage ne touche que `ACTIF`, pas `DTELASTIMPORT` : un marché absent du fichier n'a
   par définition pas été « importé » cette fois-ci. Pour toute ligne présente dans le
   fichier (ajout ou modification), `DTELASTIMPORT` = date du fichier (D1, §2/§3) est
   réécrit ; `COMPLETUDE` n'est en revanche jamais recalculée par l'import lui-même (§3).
4. **Variante marchés** (déjà prévue par OP3.1) : si `NUM_TITULAIRE` (col. D) est inconnu du
   service, auto-création d'une fiche `FOURNISSEUR` (`TYPE_CREATION = PGI`), listée dans
   « fournisseurs ajoutés ».
5. **Mise à jour de `last.import.marche.pgi`** (§7) avec la date de génération du fichier
   (cellule D1, §2) pour ce service — condition de possibilité du **prochain** import.
6. **Compte-rendu** : marchés ajoutés, archivés, fournisseurs ajoutés, anomalies — **à
   l'écran et en document téléchargeable** (décision du 29/08/2026, §8).
...
Si une de ces vérifications échoue, le processus s'arrête et un compte-rendu d'anomalie est
