...
**Intégrité référentielle** : toutes les FK des §2/§3 en FOREIGN KEY ; `ON DELETE RESTRICT` sur référentiels ; pas de suppression physique sur MARCHE (ETATMARCHE), FOURNISSEUR (ETATFOURNISSEUR), ni sur DIRECTION/SERVICE/CELLULE (ACTIF, ajout 28/08/2026 — même principe). Cas particulier : la FK composite DEMANDE_ACHAT → SOUS_SITE / SOUS_SECTEUR n'est **pas** vérifiée par Postgres quand le sous-niveau est NULL (MATCH SIMPLE) — voir note §2.4.
